	...
	namespace InSoMniA{
		...
		double zoom(double& phi_alpha, double alpha_lo, double alpha_hi,
			...
			while(true){
				...
				// std::cerr << "  begin interpolation: alpha lo: " << alpha_lo << ", alpha hi: " << alpha_hi << std::endl;
				if(alpha_lo < alpha_hi){
					alpha = cubic_interpolation(alpha_lo, alpha_hi, phi_alpha_lo, phi_alpha_hi,
					  						    d_phi_alpha_lo, d_phi_alpha_hi);
				}
				else{
					alpha = cubic_interpolation(alpha_hi, alpha_lo, phi_alpha_hi, phi_alpha_lo,
					  						    d_phi_alpha_hi, d_phi_alpha_lo);	
				}
			...
			while(true){
				...
				if(phi_alpha > phi_alpha0 + c1*alpha*d_phi_alpha0 || 
					(iter > 0 && phi_alpha > phi_alpha1)){
						// std::cout << "value too large, zoom" << std::endl;
				...
				if(d_phi_alpha >= 0){
					// std::cout << "d phi alpha larger than zero, zoom" << std::endl;
